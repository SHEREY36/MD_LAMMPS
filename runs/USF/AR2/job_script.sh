...
#SBATCH --array=0-10                   # 9 top-level folders e_060 ... e_100
#SBATCH --nodes=1
...
for folder in e_*; do
    echo "---- Running e-folder: $folder ----"
    cd "$folder"

    if [[ -f in.usf ]]; then
        srun $SLURM_SUBMIT_DIR/lmp_mpi < in.usf
    else
        echo "WARNING: in.usf not found in $folder"
    fi
