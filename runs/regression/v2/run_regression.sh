...
$LMP -in in.t2_virial -log log.t2 -screen none
for st in hertz hooke; do for a in 0.5 0.7 0.9; do
  $LMP -in in.t3_binary -var style $st -var alpha $a -log log.t3_${st}_$a -screen none
done; done
$LMP -in in.t3b_rods -log log.t3b -screen none
$LMP -in in.t7_rot_sllod -log log.t7 -screen none
$MPIRUN -np 2 $LMP -in in.t5_elastic_usf -log log.t5 -screen none
$LMP -in in.t8_decomp -var np 1 -log log.t8_np1 -screen none
...
if [ "$MODE" != "quick" ]; then
  ...
  rm -rf t6_sphere_usf
  python3 ../../fresh_usf/generate_fresh_usf.py --ARs 1 --alphas 0.7 --N 2000 --rho 1.0 \
