...
def main():
    ...
    while placed < N:
        ...
        # Check overlaps with previously placed particles (minimum image).
        # Vectorised prefilter: only centres closer than 2*hl + d can overlap.
        ok = True
        if placed > 0:
            dr_all = pos_new - positions[:placed]
            dr_all -= L * np.round(dr_all / L)
            close = np.nonzero(np.einsum('ij,ij->i', dr_all, dr_all) < (2.0*hl + d)**2)[0]
            for j in close:
                dsq = seg_seg_distsq(np.zeros(3), dr_all[j], u_new, orientations[j], hl)
                if dsq < d**2:
                    ok = False
                    break
