...
{
  // We need: rmass, omega, and quaternions (orientation).
  // PairGranHookeHistory requires radius_flag; we override that requirement.
  if (!atom->rmass_flag || !atom->omega_flag)
    error->all(FLERR, "Pair gran/spherocyl/history requires atom attributes rmass, omega");

  if (!atom->ellipsoid_flag || !atom->quat_flag)
    error->all(FLERR, "Pair gran/spherocyl/history requires atom_style ellipsoid (quat/orientation)");
  ...
  if (history && (fix_history == nullptr)) {
    auto cmd = fmt::format("NEIGH_HISTORY_SC{} all NEIGH_HISTORY {}", instance_me, size_history);
  ...
  // history fix lookup (copied from parent)
  if (history) {
    fix_history = dynamic_cast<FixNeighHistory *>(
        modify->get_fix_by_id("NEIGH_HISTORY_SC" + std::to_string(instance_me)));
    if (!fix_history) error->all(FLERR, "Could not find pair fix neigh history ID for spherocyl");
  }
}
...
void PairGranSpherocylHistory::compute(int eflag, int vflag)
{
  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      double wxc_i[3] = {
        omega[i][1]*rhoi[2] - omega[i][2]*rhoi[1],
        omega[i][2]*rhoi[0] - omega[i][0]*rhoi[2],
        omega[i][0]*rhoi[1] - omega[i][1]*rhoi[0]
      };
      double wxc_j[3] = {
        omega[j][1]*rhoj[2] - omega[j][2]*rhoj[1],
        omega[j][2]*rhoj[0] - omega[j][0]*rhoj[2],
        omega[j][0]*rhoj[1] - omega[j][1]*rhoj[0]
      };
